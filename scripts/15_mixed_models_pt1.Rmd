...
<iframe width="560" height="315" src="https://youtube.com/embed/gELAuRHCEbw" frameborder="0" allowfullscreen></iframe>
...
We first read in the datafile we need. We are also going to use `transmute()` to turn our `subject` and `gender` columns into factors.
...
<iframe width="560" height="315" src="https://youtube.com/embed/yfmqBTeyoBk" frameborder="0" allowfullscreen></iframe>
...
We can check the model assumptions using the `{performance}` package.
...
We can see with an appropriate correction for multiple comparisons (remember the familywise error rate?), that the *only* pairwise comparison that is significant is the Neutral vs Positive condition comparison.

Let's check our model assumptions - remember, we want to see the residuals normally distributed. 

```{r, warning=FALSE, message=FALSE}
check_model(factor_1_model)
```

These residuals (and other diagnostics) look pretty good to me.
